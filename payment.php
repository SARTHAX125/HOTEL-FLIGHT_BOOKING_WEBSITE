...
<div class="container">
  ...
  <?php if($totalflight !=0)
    ...
    <?php if($totalhotel !=0)
      {
     ?>
     <span class="span1"><a href ="buffer.php?username=<?php echo($username);?> & hotelname=<?php echo($hna);?> & price=<?php echo($totalhotel);?> & hotelplace=<?php echo($hdes);?> & bill=<?php echo($totalhotel);?> & in=<?php echo($in);?> & out=<?php echo($out);?> & room=<?php echo($ro);?>"></a></span>
     <?php } ?>


 <label>Card number:</label>
 <input id="user" type="text" class="input cardnumber"  placeholder="1234 XXXX XXXX XXXX" required>
 <label>Name:</label>
 <input class="input name"  placeholder="NAME" required>
 <label class="toleft">CCV:<br></label>
 <input class="input toleft ccv" placeholder="***" required>
  </div>
  <div class="receipt">
